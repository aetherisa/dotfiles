...
const float SHAKE_STRENGTH = 0.85;
...
float offsetFunction(float iTime) {
    ...
    return amount * periods[3] * SHAKE_STRENGTH;
}

const float DIM_CUTOFF = 0.35;
const float BRIGHT_CUTOFF = 0.70;
const float ABBERATION_FACTOR = 0.042;

// Glow tuning
const float SOURCE_BRIGHTEN = 1.08;
const float GLOW_STRENGTH = 0.50;
const float GLOW_CHROMA_STRENGTH = 0.18;
const float GLOW_DIM_LIGHTNESS = 0.025;
const float GLOW_BRIGHT_LIGHTNESS = 0.055;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (source.x > DIM_CUTOFF) {
        ...
        for (int i = 0; i < 24; i++) {
            ...
            if (c.x > DIM_CUTOFF) {
                glow.yz += c.yz * weight * GLOW_CHROMA_STRENGTH;

                if (c.x <= BRIGHT_CUTOFF) {
                    glow.x += c.x * weight * GLOW_DIM_LIGHTNESS;
                } else {
                    glow.x += c.x * weight * GLOW_BRIGHT_LIGHTNESS;
                }
            }
        }

        dest.xyz += glow.xyz * GLOW_STRENGTH;
    }
